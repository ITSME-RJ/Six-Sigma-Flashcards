...
function plainEnglish(c){if(!c.d)return `Connect “${c.t}” to the related ideas in Group ${c.g}.`;return c.d.replace(/^A |^An |^The /,'').replace(/\brefers to\b|\bis defined as\b|\bis the\b/i,'means').slice(0,360)}
...
function renderStudy(){
 ...
 const s=cardState(c.id);$('#favoriteBtn').classList.toggle('active',s.favorite);$('#favoriteBtn').textContent=s.favorite?'★':'☆';updateIntervals(c);
...
$('#closeStudy').onclick=()=>switchView('home');$('#flashcard').onclick=()=>$('#flashcard').classList.toggle('flipped');$('#flashcard').onkeydown=e=>{if(e.key===' '||e.key==='Enter')$('#flashcard').classList.toggle('flipped')};
...
$('#swipeStage').addEventListener('touchmove',e=>{touchDeltaX=e.touches[0].clientX-touchStartX;const el=$('#flashcard');if(Math.abs(touchDeltaX)<130)el.style.transform=`translateX(${touchDeltaX*.35}px) rotate(${touchDeltaX*.025}deg)${el.classList.contains('flipped')?' rotateY(180deg)':''}`},{passive:true});
$('#swipeStage').addEventListener('touchend',()=>{const el=$('#flashcard');el.style.transform='';if(Math.abs(touchDeltaX)>85)rateCurrent(touchDeltaX<0?'again':'good')});
